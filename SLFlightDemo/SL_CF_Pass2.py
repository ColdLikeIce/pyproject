...
def main():
    ...
    url = 'https://search.lionairthai.com/default.aspx?aid=207&depCity=PER&arrCity=PNK&Jtype=2&depDate=17/06/2024&arrDate=18/06/2024&adult1=2&child1=1&infant1=0&promotioncode=&df=UK&afid=0&b2b=0&St=fa&DFlight=false&roomcount=1&culture=en-GB&ur=lionairthai.com'
    ratecode = 'SL_PER_PNK_2024-06-17_(OD9119_JT712_202406170150-202406170850_4)|PNK_PER_2024-06-18_(JT685_OD9120_202406181345-202406190020_4)_2_1'
    ...
    try:
        ...
        if "SL" in ratecode:
            ...
            for reg in regList:
                ...
                for fly in flyinfo:
                    if '-' in fly:
                        start = str(fly).split('-')[0]
                        start = start[-4:-2] + ':' + start[-2:]
                        end = fly.split('-')[1]
                        end = end[-4:-2] + ':' + end[-2:]
                        break
                    flyMat += fly
                if index == 0:
                    alink = p.eles('css:#divOBFlightResults > div')
                else:
                    alink = p.eles('css:#divIBFlightResults > div')
                for div in alink:
                    col =  div.eles('css:.city-list >li >h4')
                    if start == col[0].text and end == col[1].text:
                        click_text = ''
                        if level == '4':
                        ...
                        if 'Sold Out' in click_text: #  卖完
                            ...
                            continue
                        if index == 0:
                            f = p.eles('css:#departDetails >span')
                        else:
                            f = p.eles('css:#returnDetails >span')
                        descmatflyno = ''
                        for a in f:
                            q =a.text
                            if 'Flight' in a.text:
                                result = re.match(r'Flight(.*?),', a.text)
                                if ':' in a.text:
                                    result = re.match(r'Flight:(.*?),', a.text)
                                flytext = result.group(1)
                                # 提取所有数字作为航班号
                                flyNo = re.search(r'\d+', flytext)
                                flyNo = flyNo.group().lstrip('0')
                                arrInfo = flytext.replace(flyNo, '')
                                descmatflyno += f'{arrInfo.strip()}{flyNo.strip()}'
                        if flyMat == descmatflyno:
                            is_match = True
                            break
                    is_match = False
        if is_match:
            summary = p.ele('text:Pricing Summary')
            ...
            if 'Adult' in lab:
                ...
                adt_price = decimal.Decimal(adt_info[1].replace(em, '')) * adt_num
            ...
            if 'Children' in lab:
                ...
                child_price = decimal.Decimal(child_info[1].replace(em, '')) * c_num
            ...
            adtPrice = baseFare / total_price * adt_price
            childPrice = baseFare-adtPrice
            adtSumPrice = adtTax + adtPrice
            childSumPrice = childTax + childPrice
            priceobj = {"adtSumPrice": str(adtSumPrice), "childSumPrice": str(childSumPrice), "adtTax": str(adtTax), "childTax": str(childTax)}
